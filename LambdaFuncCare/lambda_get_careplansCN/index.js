...
exports.handler = async (event) => {
  ...
  try {
    ...
    // SQL query to get care plans with careplan_name, client_username, status, and date_created
    const query = `
      SELECT cp.care_plan_name, cp.client_username, cp.status, cp.date_created
      FROM care_plans cp
